...
// const PropertyExamplesCarousel = dynamic(
//   () => import("../services/dynamic/PropertyExamplesCarousel"),
//   { ssr: false }
// );
...
// Dynamic import (carousel sirf popup open hone pe load hoga)
const PropertyExamplesCarousel = dynamic(
  () => import("../services/dynamic/PropertyExamplesCarousel"),
  { ssr: false }
);

export default function ServiceCardClient({ service, mobile }) {
  const [animate, setAnimate] = useState(false);
  const [openPopup, setOpenPopup] = useState({
    open: false,
    selectedService: null,
  ...
  return (
    <>
          ...
          <div className="relative overflow-hidden rounded-xl shadow-md cursor-pointer">
            ...
            >
              <div>
                <h4 className="font-semibold mb-[2%]">
                  {service.hoverDetail ||
                    service.subDescription?.slice(0, 100)}
                </h4>
      ...
      {/* Mobile Card */}
      {mobile && (
        <div className="rounded-xl shadow-md overflow-hidden bg-white lg:hidden">
          {service.leftImage || service.afterImage?.length > 0 ? (
            <SEOImage
              src={service.leftImage || service.afterImage?.[0]}
              alt={service.fieldName || service.subName}
            ...
            <div className="w-full bg-gray-200 flex items-center justify-center h-[15rem]" />
          )}
      ...
      {/* Popup */}
      {openPopup.open && openPopup.selectedService && (
        <div className="fixed inset-0 bg-white/90 backdrop-blur-sm flex justify-center items-center z-[4000]">
          <div className="container-global p-0 lg:w-[80%] xl:w-[60%] relative">

            {/* Close Button */}
            <button
              className="bg-[#00CFAA] right-[-2%] top-[-6%] w-10 h-10 flex items-center justify-center rounded-full absolute text-black text-2xl"
              onClick={() =>
                setOpenPopup({
                  open: false,
                  selectedService: null,
            ...
            <PropertyExamplesCarousel
              ...
              pricing={openPopup?.selectedService?.pricedetail}
              data={
                openPopup?.selectedService?.servicespropertexamples
              }
            />
          </div>
        </div>
